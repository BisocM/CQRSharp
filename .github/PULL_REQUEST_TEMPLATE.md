...
- [ ] `dotnet test tests/CQRSharp.Tests` passes, and the change comes with tests.
...
- [ ] **Stores:** the shared contract suites (`OutboxStoreContractTests`, `IdempotencyStoreContractTests`) pass for
      every store, and the Redis tests were run against a real server (or I am relying on CI for that).
