...
/// <summary>
///     Thrown by the validation behavior when one or more <see cref="IRequestValidator{TRequest}" />s report failures;
///     <see cref="Failures" /> holds every failure of every validator.
/// </summary>
public sealed class RequestValidationException : Exception
{
    ...
    public RequestValidationException(Type requestType, IReadOnlyList<ValidationFailure> failures)
        : base($"Validation failed for request '{(requestType ?? throw new ArgumentNullException(nameof(requestType))).Name}'.")
    {
