...
internal static class ContractAssert
{
    ...
    public static void SequenceEqual(byte[] expected, byte[] actual, string what)
    {
        if (!expected.AsSpan().SequenceEqual(actual))
            Assert.Fail($"{what}: expected {expected.Length} byte(s) [{FormatAll(expected)}], but found {actual.Length} byte(s) [{FormatAll(actual)}].");
    }
