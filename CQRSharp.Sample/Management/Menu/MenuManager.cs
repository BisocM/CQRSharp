    ...
    public class MenuManager
    {
        ...
        private void SetupMenus()
        {
            ...
            foreach (var type in commandTypes)
            {
                ...
                _logger.LogDebug($"Registered command {type.Name} with priority {attribute.Priority} in menu {attribute.MenuId}.");
            }

            _logger.LogInformation($"Menu setup complete with {_menus.Count} menus and {_menus.Sum(x => x.Value.Count)} commands.");
        }
        ...
        private (Type commandType, int priorityvoid) RetrieveUserSelection(List<(Type commandType, int priority)>? sortedMenuItems)
        {
            string menuDisplay = sortedMenuItems.Aggregate("", (current, menuItem) => current + $"{menuItem.priority}. {FormatCommandName(menuItem.commandType.Name)}\n");
            ...
            int.TryParse(userSelection, out int selection);
