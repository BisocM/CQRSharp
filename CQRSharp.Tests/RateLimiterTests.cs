...
public class RateLimitingBehaviorTests
{
    ...
    public async Task DifferentUsers_IndependentBuckets()
    {
        ...
        //Exhaust u6
        for (var i = 0; i < _options.MaxTokens; i++)
            await _behavior.Handle(new TestCmd { Context = ctxA }, _ => Task.FromResult<object>(null!),
                CancellationToken.None);

        //u7 should still succeed
        Func<Task> actB = () => _behavior.Handle(new TestCmd { Context = ctxB }, _ => Task.FromResult<object>(null!),
            CancellationToken.None);
