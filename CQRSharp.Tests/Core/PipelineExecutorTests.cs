...
public class PipelineExecutorTests
{
    ...
    public async Task ExecuteCommandAsync_InvokesHandlerAndPublishesNotifications()
    {
        ...
        _mockNotificationDispatcher.Verify(d => d.Publish(It.IsAny<CQRSharp.Core.Notifications.Types.CommandInitiatedNotification>(), It.IsAny<CancellationToken>()), Times.Once);
        _mockNotificationDispatcher.Verify(d => d.Publish(It.IsAny<CQRSharp.Core.Notifications.Types.CommandCompletedNotification>(), It.IsAny<CancellationToken>()), Times.Once);
    }
