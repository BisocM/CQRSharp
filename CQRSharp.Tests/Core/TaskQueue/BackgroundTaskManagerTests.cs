...
public class BackgroundTaskManagerTests
{
        ...
        BackgroundTaskQueueOptions opts, ILogger<BackgroundTaskQueue>? logger = null)
    {
        ...
        var queue = new BackgroundTaskQueue(Opts(opts), dispatcher, lifetime, metrics, queueLogger);
    ...
    public async Task NotificationDispatch_RetriesOnFailureAndThenSucceeds()
    {
        ...
        var dispatcher = new ControllableDispatcher(failCount: 2); // Fails twice, succeeds on the 3rd try
        var queue = new BackgroundTaskQueue(Opts(opts), dispatcher, new TestHostApplicationLifetime(), new TestMetricsReporter(), NullLogger<BackgroundTaskQueue>.Instance);
