...
- "Stream (3 items)" dispatches a streaming request and enumerates its three items to the end: the dispatch plus the
  per-item cost of whatever each library wraps around the handler's `IAsyncEnumerable`.
