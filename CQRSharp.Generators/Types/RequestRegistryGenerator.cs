...
public sealed class RequestRegistryGenerator : IIncrementalGenerator
{
        ...
        ImmutableArray<INamedTypeSymbol?> candidateClasses)
    {
        ...
        var preHandlerInterfaceSymbol =
            compilation.GetTypeByMetadataName("CQRSharp.Core.Pipelines.Attributes.IPreHandlerAttribute");
        var postHandlerInterfaceSymbol =
            compilation.GetTypeByMetadataName("CQRSharp.Core.Pipelines.Attributes.IPostHandlerAttribute");
        var pipelineExemptionAttributeSymbol =
            compilation.GetTypeByMetadataName("CQRSharp.Core.Pipelines.Attributes.Markers.PipelineExemptionAttribute");
        ...
        foreach (var candidate in candidateClasses.Distinct())
        {
            ...
            foreach (var iface in candidate.AllInterfaces)
            {
                ...
                var preHandlersCode = GenerateAttributeArrayCode(requestTypeSymbol, preHandlerInterfaceSymbol,
                    typeof(IPreHandlerAttribute).FullName ?? throw new InvalidOperationException("Could not resolve IPreHandlerAttribute type name."));
                var postHandlersCode = GenerateAttributeArrayCode(requestTypeSymbol, postHandlerInterfaceSymbol,
                    typeof(IPostHandlerAttribute).FullName ?? throw new InvalidOperationException("Could not resolve IPostHandlerAttribute type name."));
                var pipelineExemptionsCode = GenerateAttributeArrayCode(requestTypeSymbol,
                    pipelineExemptionAttributeSymbol,
                    typeof(PipelineExemptionAttribute).FullName ?? throw new InvalidOperationException("Could not resolve PipelineExemptionAttribute type name."));
    ...
    {
        string propertySensitivityFullyQualifiedName = typeof(PropertySensitivity).FullName ?? throw new InvalidOperationException("Could not resolve PropertySensitivity type name.");
        ...
        return !sensitiveProps.Any() ? "Array.Empty<{propertySensitivityFullyQualifiedName}>()" : $"new {propertySensitivityFullyQualifiedName}[] {{ {string.Join(", ", sensitiveProps)} }}";
    }
    ...
    private static bool InheritsOrImplements(ITypeSymbol type, INamedTypeSymbol baseType)
    {
        ...
        if (Enumerable.Any(type.AllInterfaces, iface => SymbolEqualityComparer.Default.Equals(iface, baseType)))
            return true;
