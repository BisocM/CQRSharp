...
{
    /// <summary>
    /// A source generator that discovers types implementing ICommandHandler and IQueryHandler (both decorated with [HandlerType]),
    /// and generates a static registry mapping request types to their corresponding invocation delegates.
    /// </summary>
    [Generator]
    public sealed class HandlerRegistryGenerator : IIncrementalGenerator
    {
            ...
            ImmutableArray<INamedTypeSymbol?> classes)
        {
            ...
            const string handlerTypeAttrName = "CQRSharp.Shared.Attributes.Requests.HandlerTypeAttribute";
            ...
            foreach (var classSymbol in classes)
            {
                ...
                //Examine every interface implemented by the class.
                foreach (var iface in classSymbol.AllInterfaces)
                {
                    //Look for interfaces decorated with [HandlerType].
                    bool hasHandlerTypeAttr = iface.GetAttributes()
                        .Any(attr => SymbolEqualityComparer.Default.Equals(attr.AttributeClass, handlerTypeAttributeSymbol));
                    if (!hasHandlerTypeAttr)
                        continue;
                    ...
                    if (iface.Name.StartsWith("ICommandHandler", StringComparison.Ordinal))
                    {
                        ...
                        {
                            //If there's no type argument, skip or emit a diagnostic if desired.
                            continue;
                        }

                        var requestType = iface.TypeArguments[0].ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat);
                        //For commands, we assume a fixed result type of CommandResult.
                        var resultType = "global::CQRSharp.Data.Commands.CommandResult";

                        registrationsBuilder.Add(new HandlerRegistrationEntry(
                            requestType,
                            resultType,
                            isQuery: false
                        ));
                    ...
                    else if (iface.Name.StartsWith("IQueryHandler", StringComparison.Ordinal))
                    {
                        ...
                        var requestType = iface.TypeArguments[0].ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat);
                        var resultType = iface.TypeArguments[1].ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat);
        ...
        private static string GenerateRegistrySource(ImmutableArray<HandlerRegistrationEntry> registrations)
        {
            ...
            sb.AppendLine();
            sb.AppendLine("        /// <summary>");
            sb.AppendLine("        /// Initializes a new instance of the <see cref=\"GeneratedHandlerRegistryRegistrar\"/> class.");
            sb.AppendLine("        /// </summary>");
            ...
            foreach (var reg in distinctRegs)
            {
                ...
                if (reg.IsQuery)
                {
                    ...
                    sb.AppendLine("            {");
                    sb.AppendLine($"                return ((global::CQRSharp.Interfaces.Handlers.IQueryHandler<{reg.RequestType}, {reg.ResultType}>)handler)");
                ...
                else
                {
                    sb.AppendLine($"            _handlerMap.TryAdd(typeof({reg.RequestType}), (handler, request, ct) =>");
                    sb.AppendLine("            {");
                    sb.AppendLine($"                return ((global::CQRSharp.Interfaces.Handlers.ICommandHandler<{reg.RequestType}>)handler)");
                    sb.AppendLine($"                    .Handle(({reg.RequestType})request, ct)");
                    sb.AppendLine("                    .ContinueWith(t => (object)t.Result, ct);");
                    sb.AppendLine("            });");
                }
            ...
            sb.AppendLine();
            sb.AppendLine("    /// <summary>");
            sb.AppendLine("    /// A module initializer that assigns the generated handler registry to the global Registrar.");
            sb.AppendLine("    /// </summary>");
            ...
            sb.AppendLine("            {");
            sb.AppendLine("                throw new InvalidOperationException(\"Failed to initialize GeneratedHandlerRegistry.\", ex);");
            sb.AppendLine("            }");
            sb.AppendLine("        }");
            sb.AppendLine("    }");
            sb.AppendLine("}");
        ...
        /// </summary>
        private sealed class HandlerRegistrationEntry : IEquatable<HandlerRegistrationEntry>
        {
            public HandlerRegistrationEntry(string requestType, string resultType, bool isQuery)
            {
            ...
            public override bool Equals(object? obj) => Equals(obj as HandlerRegistrationEntry);
            ...
            public override int GetHashCode()
            {
                unchecked
                {
                    ...
                    hash = (hash * 23) + ResultType.GetHashCode();
        ...
        private static class HandlerRegistryDiagnostics
        {
            ...
            public static readonly DiagnosticDescriptor MissingHandlerTypeAttribute = new(
                id: "CQRHRG001",
                title: "Missing HandlerTypeAttribute",
                messageFormat: "Could not find the HandlerTypeAttribute. Ensure CQRSharp.Shared.Attributes is referenced.",
            ...
            public static readonly DiagnosticDescriptor NoHandlerRegistrationsFound = new(
                ...
                messageFormat: "No handler registrations were discovered. Ensure that your handlers implement interfaces decorated with [HandlerType].",
                category: "CQRSharp.Generators",
            ...
            public static readonly DiagnosticDescriptor UnhandledException = new(
                id: "CQRHRG999",
                title: "Unhandled Exception in HandlerRegistryGenerator",
