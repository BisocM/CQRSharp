...
namespace CQRSharp.Generators.Types
{
    ...
    public sealed class HandlerRegistryGenerator : IIncrementalGenerator
    {
            ...
            ImmutableArray<INamedTypeSymbol?> classes)
        {
            ...
            foreach (var iface in from classSymbol in Enumerable.OfType<INamedTypeSymbol>(classes) from iface in classSymbol.AllInterfaces let hasHandlerTypeAttr = iface.GetAttributes()
                         .Any(attr =>
                             SymbolEqualityComparer.Default.Equals(attr.AttributeClass, handlerTypeAttributeSymbol)) where hasHandlerTypeAttr select iface)
            {
                ...
                if (iface.Name.StartsWith("ICommandHandler", StringComparison.Ordinal))
                {
                    ...
                    var handlerInterface = $"global::CQRSharp.Shared.Data.Interfaces.Handlers.ICommandHandler<{string.Join(", ", genericArgs)}>";
                    registrationsBuilder.Add(new HandlerRegistrationEntry(requestType, handlerInterface, isQuery: false));
                }
                ...
                else if (iface.Name.StartsWith("IQueryHandler", StringComparison.Ordinal))
                {
                    ...
                    var requestType = genericArgs[0];
                    var handlerInterface = $"global::CQRSharp.Shared.Data.Interfaces.Handlers.IQueryHandler<{string.Join(", ", genericArgs)}>";
                    registrationsBuilder.Add(new HandlerRegistrationEntry(requestType, handlerInterface, isQuery: true));
                }
            }
