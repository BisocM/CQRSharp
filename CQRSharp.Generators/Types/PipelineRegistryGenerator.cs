...
namespace CQRSharp.Generators.Types
{
    ...
    public sealed class PipelineRegistryGenerator : IIncrementalGenerator
    {
            ...
            ImmutableArray<INamedTypeSymbol?> types)
        {
            ...
            var markerFullName = typeof(Shared.Attributes.Requests.RequestMarkerAttribute).FullName;
        ...
        private static string InferResultType(INamedTypeSymbol requestSymbol)
        {
            ...
            if (iCommand != null)
            {
                return "global::CQRSharp.Data.Commands.CommandResult";
            }
        ...
        private static string GenerateRegistrarSource(ImmutableArray<(string RequestType, string ResultType)> entries)
        {
            ...
            sb.AppendLine("    /// A generated registrar that registers an implementation of IPipelineRegistry with the DI container.");
            sb.AppendLine("    /// </summary>");
            ...
            sb.AppendLine();
            sb.AppendLine("        /// <summary>");
            sb.AppendLine("        /// Initializes a new instance of the <see cref=\"GeneratedPipelineRegistrar\"/> class.");
            sb.AppendLine("        /// </summary>");
            ...
            {
                sb.AppendLine($"            _pipelineMap.TryAdd(typeof({requestType}), (services, requestObj, finalHandler, ct) =>");
                sb.AppendLine("            {");
                ...
                sb.AppendLine("                }");
                sb.AppendLine();
                sb.AppendLine("                return pipeline((requestObj as " + requestType + ")!, ct).ContinueWith(t => (object)t.Result, ct);");
                sb.AppendLine("            });");
                sb.AppendLine();
            }
            ...
            sb.AppendLine();
            sb.AppendLine("    /// <summary>");
            sb.AppendLine("    /// A module initializer that assigns the generated pipeline registrar to the global Registrar.");
            sb.AppendLine("    /// </summary>");
            ...
            sb.AppendLine("            {");
            sb.AppendLine("                throw new InvalidOperationException(\"Failed to initialize GeneratedPipelineRegistrar.\", ex);");
            sb.AppendLine("            }");
            sb.AppendLine("        }");
            sb.AppendLine("    }");
            sb.AppendLine("}");
