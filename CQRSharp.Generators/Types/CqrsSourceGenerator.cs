...
public sealed class CqrsSourceGenerator : IIncrementalGenerator
{
        ...
        ImmutableArray<INamedTypeSymbol> candidateClasses)
    {
        ...
        GenerateNotificationSerializer(sb, compilation, candidateClasses);
        ...
        foreach (var candidate in candidateClasses.Distinct(SymbolEqualityComparer.Default).OfType<INamedTypeSymbol>())
        {
                         ...
                         i.GetAttributes().Any(attr => SymbolEqualityComparer.Default.Equals(attr.AttributeClass, handlerTypeAttributeSymbol))))
            {
                ...
                var contextTypeSymbol = GetRequestContextType(requestTypeSymbol) ??
                                        compilation.GetTypeByMetadataName("CQRSharp.Abstractions.Data.Interfaces.Context.RequestContextBase");
    ...
    private static void GenerateNotificationSerializer(StringBuilder sb, Compilation compilation, ImmutableArray<INamedTypeSymbol> candidateClasses)
    {
        sb.AppendLine();
        sb.AppendLine("            // Registering Notification Serializer");

        var iNotificationSymbol = compilation.GetTypeByMetadataName("CQRSharp.Abstractions.Data.Interfaces.Notifications.INotification");
        if (iNotificationSymbol is null)
        {
            sb.AppendLine("            // INotification interface not found, skipping serializer generation.");
            return;
        }

        var notifications = candidateClasses
            .Where(c => c is { IsAbstract: false } && c.AllInterfaces.Contains(iNotificationSymbol, SymbolEqualityComparer.Default))
            .Distinct(SymbolEqualityComparer.Default)
        ...
        {
            sb.AppendLine("            // No concrete INotification types found, skipping serializer generation.");
            return;
        }

        sb.AppendLine(
            "            services.AddSingleton<global::CQRSharp.Abstractions.Data.Interfaces.Notifications.INotificationSerializer>(new CQRSharp.Core.Extensions.Generated.GeneratedNotificationSerializer());");

        var generatedCode = new StringBuilder();
        generatedCode.AppendLine("// <auto-generated />");
        generatedCode.AppendLine("namespace CQRSharp.Core.Extensions.Generated");
        ...
        generatedCode.AppendLine();
        generatedCode.AppendLine("    file sealed class GeneratedNotificationSerializer : global::CQRSharp.Abstractions.Data.Interfaces.Notifications.INotificationSerializer");
        generatedCode.AppendLine("    {");

        // Serialize method
        generatedCode.AppendLine("        public string Serialize(global::CQRSharp.Abstractions.Data.Interfaces.Notifications.INotification notification)");
        generatedCode.AppendLine("        {");
        generatedCode.AppendLine("            switch (notification)");
        generatedCode.AppendLine("            {");
        foreach (var notificationSymbol in notifications)
        {
            var typeName = notificationSymbol.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat);
            generatedCode.AppendLine($"                case {typeName} n:");
            generatedCode.AppendLine("                {");
            generatedCode.AppendLine("                    var parts = new List<string>();");
            var props = GetSerializableProperties(notificationSymbol);
            foreach (var prop in props) generatedCode.AppendLine($"                    parts.Add($\"{prop.Name}={{EncodeValue(n.{prop.Name})}}\");");

            generatedCode.AppendLine("                    return string.Join(\"\\n\", parts);");
            generatedCode.AppendLine("                }");
        }

        generatedCode.AppendLine("                default:");
        generatedCode.AppendLine(
            "                    throw new NotSupportedException($\"Notification type {notification.GetType().FullName} is not supported for serialization.\");");
        generatedCode.AppendLine("            }");
        generatedCode.AppendLine("        }");
        generatedCode.AppendLine();

        // Deserialize method
        generatedCode.AppendLine("        public global::CQRSharp.Abstractions.Data.Interfaces.Notifications.INotification Deserialize(string notificationType, string payload)");
        generatedCode.AppendLine("        {");
        generatedCode.AppendLine("            var properties = payload.Split(new[] { '\\n' }, StringSplitOptions.RemoveEmptyEntries)");
        generatedCode.AppendLine("                .Select(p => p.Split(new[] { '=' }, 2))");
        generatedCode.AppendLine("                .ToDictionary(p => p[0], p => DecodeValue(p[1]));");
        generatedCode.AppendLine();
        generatedCode.AppendLine("            switch (notificationType)");
        generatedCode.AppendLine("            {");
        foreach (var notificationSymbol in notifications)
        {
            var typeName = notificationSymbol.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat);
            generatedCode.AppendLine($"                case \"{notificationSymbol.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat)}\":");
            generatedCode.AppendLine("                {");

            var ctor = notificationSymbol.Constructors.OrderByDescending(c => c.Parameters.Length).FirstOrDefault();
            if (ctor is null || !ctor.Parameters.Any())
            {
                generatedCode.AppendLine(
                    $"                     throw new NotSupportedException(\"Notification type {typeName} must have a public constructor with parameters to be deserialized.\");");
            }
            else
            {
                generatedCode.AppendLine($"                    return new {typeName}(");
                var ctorParams = ctor.Parameters;
                for (var i = 0; i < ctorParams.Length; i++)
                {
                    var param = ctorParams[i];
                    var pascalCasePropName = char.ToUpperInvariant(param.Name[0]) + param.Name.Substring(1);
                    var parseLogic = GetParseLogic(param.Type, $"properties[\"{pascalCasePropName}\"]");
                    generatedCode.Append($"                        {param.Name}: {parseLogic}");
                    generatedCode.AppendLine(i < ctorParams.Length - 1 ? "," : "");
                }
        ...
        generatedCode.AppendLine("                default:");
        generatedCode.AppendLine("                    throw new NotSupportedException($\"Notification type {notificationType} is not supported for deserialization.\");");
        generatedCode.AppendLine("            }");
        generatedCode.AppendLine("        }");
        generatedCode.AppendLine();

        // Helper methods for serialization
        generatedCode.AppendLine("        private static string EncodeValue(object obj)");
        generatedCode.AppendLine("        {");
        generatedCode.AppendLine("            if (obj is null) return string.Empty;");
        generatedCode.AppendLine(
            "            var str = obj switch { DateTime dt => dt.ToString(\"o\", CultureInfo.InvariantCulture), _ => Convert.ToString(obj, CultureInfo.InvariantCulture) };");
        generatedCode.AppendLine("            return Convert.ToBase64String(System.Text.Encoding.UTF8.GetBytes(str));");
        generatedCode.AppendLine("        }");
        generatedCode.AppendLine();
        generatedCode.AppendLine("        private static string DecodeValue(string base64)");
        generatedCode.AppendLine("        {");
        generatedCode.AppendLine("            if (string.IsNullOrEmpty(base64)) return null;");
        generatedCode.AppendLine("            var bytes = Convert.FromBase64String(base64);");
        generatedCode.AppendLine("            return System.Text.Encoding.UTF8.GetString(bytes);");
        generatedCode.AppendLine("        }");
    ...
    {
        // For records, constructor parameters are the source of truth.
        var ctor = typeSymbol.Constructors.OrderByDescending(c => c.Parameters.Length).FirstOrDefault();
        if (ctor is null) yield break;

        var props = typeSymbol.GetMembers().OfType<IPropertySymbol>().ToDictionary(p => p.Name, p => p, StringComparer.OrdinalIgnoreCase);
        foreach (var param in ctor.Parameters)
            if (props.TryGetValue(param.Name, out var prop))
                yield return prop;
    }

    private static string GetParseLogic(ITypeSymbol type, string value)
    {
        var typeName = type.ToDisplayString();
        return typeName switch
        {
            "string" => value,
            "int" or "System.Int32" => $"int.Parse({value}, CultureInfo.InvariantCulture)",
            "long" or "System.Int64" => $"long.Parse({value}, CultureInfo.InvariantCulture)",
            "bool" or "System.Boolean" => $"bool.Parse({value})",
            "System.Guid" => $"Guid.Parse({value})",
            "System.DateTime" => $"DateTime.Parse({value}, CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind)",
            "System.DateTimeOffset" => $"DateTimeOffset.Parse({value}, CultureInfo.InvariantCulture)",
            "decimal" or "System.Decimal" => $"decimal.Parse({value}, CultureInfo.InvariantCulture)",
            "double" or "System.Double" => $"double.Parse({value}, CultureInfo.InvariantCulture)",
            "float" or "System.Single" => $"float.Parse({value}, CultureInfo.InvariantCulture)",
            _ => type.TypeKind == TypeKind.Enum
                ? $"({typeName})Enum.Parse(typeof({typeName}), {value})"
                : $"({typeName})Convert.ChangeType({value}, typeof({typeName}), CultureInfo.InvariantCulture)"
        };
    }
