    ...
    public sealed class RequestDispatcher : IRequestDispatcher
    {
        ...
        public Task<CommandResult> ExecuteCommand(ICommand command, CancellationToken cancellationToken = default)
        {
                ...
                try
                {
                        ...
                        _backgroundTaskQueue is BackgroundTaskQueue concrete)
                    {
                        concrete.RegisterTaskCompletion(
                            ...
                            cancelAction: () => tcs.TrySetCanceled(),
                            exceptionAction: ex   => tcs.TrySetException(ex));
                    }
            ...
            async Task<CommandResult> PipelineTask(CancellationToken ct)
            {
                ...
                await _notificationDispatcher.Publish(new CommandCompletedNotification(command, result), ct).ConfigureAwait(false);
        ...
        public Task<TResult?> ExecuteQuery<TResult>(IQuery<TResult> query, CancellationToken cancellationToken = default)
        {
            ...
            async Task<TResult?> PipelineQueryTask(CancellationToken ct)
            {
                ...
                await _notificationDispatcher.Publish(new QueryInitiatedNotification<TResult>(query), ct).ConfigureAwait(false);
                ...
                await _notificationDispatcher.Publish(new QueryCompletedNotification<TResult>(query, result), ct).ConfigureAwait(false);
            ...
            foreach (var attr in metadata.PreHandlers)
            {
                ...
                catch (Exception ex)
                {
                    throw new InvalidOperationException(
                        $"Error in pre-handle attribute '{attr.GetType().Name}' for '{request.GetType().Name}': {ex.Message}", ex);
                }
            }
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            foreach (var attr in metadata.PostHandlers)
            {
                ...
                {
                    throw new InvalidOperationException(
                        $"Error in post-handle attribute '{attr.GetType().Name}' for '{request.GetType().Name}': {ex.Message}", ex);
                }
            }
        }
